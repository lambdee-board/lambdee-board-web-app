...
interface Props {
  ...
  dataLoadingOrError: boolean
  updateFilters: (filters: Filters | Record<string, unknown>) => void
}

const UsersFilter = ({ workspaces, dataLoadingOrError, updateFilters }: Props) => {
  const { workspaceId } = useParams()
  const [nameSearch, setNameSearch] = React.useState('')
  const [workspaceField, setWorkspaceField] = React.useState<number | string>('')
  ...
  React.useEffect(() => {
    if (!dataLoadingOrError && workspaces.length > 0 && workspaceId) {
      setWorkspaceField(Number.parseInt(workspaceId))
    }
  }, [dataLoadingOrError, workspaces.length, workspaceId])
    ...
    <div className='UsersFilter-wrapper'>
      ...
      <FormControl className='formControls'>
        ...
        >
          ...
          { !dataLoadingOrError && workspaces.map((workspace, idx) => (
            <MenuItem
              value={workspace.id}
