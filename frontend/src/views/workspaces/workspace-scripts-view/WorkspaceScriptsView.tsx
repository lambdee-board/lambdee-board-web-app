...
export default function WorkspaceScriptsView() {
  ...
  const persistNewScript = (event: React.FormEvent, script: unknown) => {
    event.preventDefault()
    saveScript(script)
    closeNewScript()
  }

  const saveScript = (payload: unknown) => {
    apiClient.post('/api/scripts', payload)
      .then((response) => {
        addAlert({ severity: 'success', message: `Added ${(payload as { name: string }).name}` })
  ...
  const persistNewScriptVariable = (event: React.FormEvent, scriptVariable: unknown) => {
    event.preventDefault()
    saveScriptVariable(scriptVariable)
    closeNewScriptVariable()
  }

  const saveScriptVariable = (payload: unknown) => {
