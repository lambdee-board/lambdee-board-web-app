...
export default function EditScriptCodeView() {
  ...
  const [outputHistory, setOutputHistory] = React.useState<OutputEntry[]>([{
  ...
  const openWsConnection = () => {
    ...
    newWebSocket.onmessage = async (event) => {
      ...
      case WebSocketMessage.types.consoleOutput:
        addToOutputHistory(message.payload as string)
        break
      case WebSocketMessage.types.consoleOutputEnd:
        if (message.payload) addToOutputHistory(message.payload as string)
        newWebSocket.close()
        break
      }
    }
  ...
  const addToOutputHistory = (content: string) => {
