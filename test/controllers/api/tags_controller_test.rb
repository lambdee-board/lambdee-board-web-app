    ...
    context 'developer' do
      ...
      should 'create tag without task given' do
        assert_difference('DB::Tag.count') do
          post api_tags_url(@board), params: { tag: { color: @tag.color, name: @tag.name, board_id: @board.id } }, headers: auth_headers(@user), as: :json
        end

        assert_response :created
        json = ::JSON.parse(response.body)
        assert_equal @tag.color, json['color']
      ...
      should 'create tag without task_id given on /api/boards/:board_id/tags' do
        assert_difference('DB::Tag.count') do
          post api_board_tags_url(@board), params: { tag: { color: @tag.color, name: @tag.name } }, headers: auth_headers(@user), as: :json
        end
      ...
      should 'create tag without task_id given on /api/boards/:task_id/tags' do
        assert_difference('DB::Tag.count') do
          post api_task_tags_url(@task), params: { tag: { color: @tag.color, name: @tag.name } }, headers: auth_headers(@user), as: :json
        end
      ...
      should 'ignore board_id if task_id is given' do
        ...
        post api_task_tags_url(@task), params: { tag: { color: @tag.color, name: @tag.name, board_id: board2.id  } }, headers: auth_headers(@user), as: :json
      ...
      should 'update tag' do
        patch api_tag_url(@tag), params: { tag: { board_id: @tag.board_id, color: @tag.color, name: 'val de larmes' } }, headers: auth_headers(@user), as: :json
