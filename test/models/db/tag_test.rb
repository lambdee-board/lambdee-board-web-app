...
class DB::TagTest < ActiveSupport::TestCase
  ...
  should 'not save when color field length is not between 7 and 9 chars' do
    tag = ::FactoryBot.build(:tag, color: 'a' * 6 )
    tag.save
    assert_equal 'Color is too short (minimum is 7 characters)', tag.errors.full_messages.first
    tag.color = 'a' * 10
    tag.save
    assert_equal 'Color is too long (maximum is 9 characters)', tag.errors.full_messages.first
    assert_not tag.persisted?
  end
